...
goog.provide('Blockly.cpp.stdio');

goog.require('Blockly.cpp');

Blockly.cpp['library_stdio_printf'] = function(block) {
    // Print statement
    var argument = '';
    var typeCode = '';
    var inQutCode = '';
    ...
    for (var n = 0; n <= block.printAddCount_; n++) {
        ...
        if(childBlock){
            ...
            else if (childBlockType == 'variables_array_get')
            {
                ...
                typeCode = Blockly.cpp.varTypeCheckInPrintScan(tempArgu1[0]);
            ...
            else if (childBlockType == 'variables_pointer_&')
            {
                if(childBlock.inputList[0].connection.targetBlock()){
                    argument = Blockly.cpp.valueToCode(childBlock, 'VALUE', Blockly.cpp.ORDER_NONE) || '';
            ...
            else if (childBlockType == 'variables_pointer_*')
            {
                if(childBlock.inputList[0].connection.targetBlock()){
                    ...
                    if(argument.indexOf('*') >= 0 && childBlock.inputList[0].connection.targetBlock().inputList[0].connection.targetBlock()){ // when double pointer
                        var astChild = Blockly.cpp.valueToCode(childBlock.inputList[0].connection.targetBlock(), 'VALUE', Blockly.cpp.ORDER_NONE) || '';

                        typeCode = Blockly.cpp.pointerTypeCheckInPrint(astChild, true);
                        if (typeCode == '') {
                            inQutCode += astChild;
                        } else {
                            inQutCode += typeCode;
                        ...
                        typeCode = Blockly.cpp.pointerTypeCheckInPrint(argument, false);
                        if (typeCode == '') {
                            inQutCode += argument;
                        } else {
                            inQutCode += typeCode;
            ...
            {
                typeCode = Blockly.cpp.varTypeCheckInPrintScan(argument);
    ...
    Blockly.cpp.definitions_['include_cpp_stdio'] =
        '#include <stdio.h>';
    return code + '\n';
};

Blockly.cpp['library_stdio_text'] = function(block) {
    // Text value.
...
Blockly.cpp['library_stdio_newLine'] = function() {
    // new line block for '\n'
    var code = '\\n';
    return [code, Blockly.cpp.ORDER_NONE];
};

Blockly.cpp['library_stdio_scanf'] = function(block) {
    // Scan statement
    var argument = '';
    var typeCode = '';
    var inQutCode = '';
    ...
    for (var n = 0; n <= block.scanAddCount_; n++) {
        ...
        if(childBlock){
            ...
            if (childBlockType == 'variables_array_get')
            {
                ...
                typeCode = Blockly.cpp.arrTypeCheckInScan(tempArgu1[0], childConnection);
...
Blockly.cpp.varTypeCheckInPrintScan = function(varName) { // variable type check
    var typeCode = '';
...
Blockly.cpp.pointerTypeCheckInPrint = function(varName, checkDoubleAst) { // pointer type check
    var typeCode = '';
    var varList = Blockly.Variables.allVariables();
...
Blockly.cpp.arrTypeCheckInScan = function(varName, childConnection) {
    var typeCode = '';
    var childBlock = childConnection.targetBlock();
