...
/**
 * @fileoverview Helper functions for generating C++ for blocks. Modified from the standard Blockly cpp generator. 
...
Blockly.cpp.finish = function(code) {
  ...
  code = '\n' + code;
    ...
    var allDefs = allFuncs.length > 5 ? '#include <bits/stdc++.h>\n\nusing namespace std;' : '' + '\n\n' + declarations.join('\n') + '\n\n' + defines.join('\n');
...
// check if cin is use
/**
 * check if cin is used
 * @returns {boolean} whether cin is used
 */
Blockly.cpp.usedCin = function() {
  var blockList = Blockly.mainWorkspace.getAllBlocks();
  for(var i in blockList) {
    if(blockList[i].type == "library_iostream_cin") {
      console.log('use cin');
      return true;
    }
  }
  console.log('not use cin');
  return false;
}