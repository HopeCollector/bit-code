...
function maybeAddClosureLibrary(srcs) {
  if (argv.closureLibrary) {
    ...
    srcs.push('./node_modules/google-closure-library/**/*.js');
  }
  console.log(srcs)
  return srcs;
}
...
function buildCompressed(cb) {
  ...
  return gulp.src(maybeAddClosureLibrary(['core/*.js']), {base: './'})
    // .pipe(through2.obj(function (file, encode, cb) {
    ...
    .pipe(compile({
      ...
      define: defines,
      language_in:
        argv.closureLibrary ? 'ECMASCRIPT_NEXT' : 'ECMASCRIPT5_STRICT'
...
goog.provide('Blockly.utils.string');`;
  return gulp.src(['blocks/*.js'], {base: './'})
    ...
    .pipe(gulp.replace(`goog.provide('Blockly.Blocks.colour');`,
      `${provides}goog.provide('Blockly.Blocks.colour');`))
    .pipe(stripApacheLicense())
    .pipe(compile({
